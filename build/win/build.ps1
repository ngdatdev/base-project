...
# Run the dotnet restore command to restore dependencies
if ($LASTEXITCODE -ne 0) {
    Write-Error "dotnet restore failed"  
    # If restore command fails
    exit $LASTEXITCODE  
    # Exit with the restore command's exit code
}

# Build the project
Write-Output "Build project..."
dotnet build --no-restore -c $CONFIGURATION_MODE $SOLUTION_FILE 
# Run the dotnet build command with the Release configuration
if ($LASTEXITCODE -ne 0) {
